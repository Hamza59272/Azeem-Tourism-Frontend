...
export default function AddressForm({
  ...
  setMilestones
    ...
    const handleAddField = () => {
        setMilestones([...milestones, {title: "" , image: ""}]); 
  ...
  const handleUploadImageForMilestone = (milestoneIndex) => {
    let widget = window.cloudinary.createUploadWidget(
      {
        cloudName: "dhhjr3vim",
        uploadPreset: "nnitbvl3",
      ...
      (error, result) => {
        if (!error && result && result.event === "success") {
          ...
          newMilestones[milestoneIndex].image = result.info.url;
  ...
  return (
    <React.Fragment>
       ...
       {milestones.map((milestone, index) => (
                    <Grid item lg={6} md={6} sm={6} xs={12} key={index}>
                        ...
                        <Box>
                            <label htmlFor="icon-button-file">
                            <IconButton
                                    onClick={() => handleUploadImageForMilestone(index)}
                                    color="primary"
                                    aria-label="upload picture"
                                    component="span"
                                    disabled={milestones[index].image != ''}
                                >
                                <Button
                                    style={{ width: "110px" }}
                                    variant="contained"
     ...
     <Button variant="contained" onClick={handleAddField} 
           sx={{ mx: {lg :3 , md : 3 , sm : 3 , xs : 'auto'} ,mb:5, mt:10, padding:1 }}>
        Add New Milestone
      </Button>
     </Grid>
